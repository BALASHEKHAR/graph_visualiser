...
export const Node = (props) => {
  ...
  const { node, edges, handleMove, handleEdge, deleteNodeMode,
    deleteEdgeMode, editEdgeMode, readyForVisualization,
    readyForMovement, readyForEdge, pathFindingNode, } = props;
  return (
    <g className={styles.nodeGroup}>
      <circle
        onPointerDown={handleMove}
        className={`${styles.node} ${deleteNodeMode && styles.deleteNodeMode} ${node.isVisited && styles.visited
          } ${node.isInShortestPath && styles.shortestPath} ${readyForVisualization && styles.readyForVisualization
          } ${pathFindingNode &&
          ...
          return (
            <>
              {edge.type === "directed" && (
                <>
                  <marker
                    className={`${styles.arrow} ${edge.isUsedInTraversal && styles.arrowTraversal
                  ...
                  <path
                    ...
                    d={directedPath}
                    className={`${styles.directedEdge} ${deleteEdgeMode && styles.deleteEdgeMode
                      } ${editEdgeMode && styles.editEdgeMode} ${(readyForVisualization ||
                        readyForMovement ||
                        readyForEdge) &&
                      styles.disableEdge
                      } ${edge.isUsedInTraversal && styles.usedInTraversal} ${edge.isUsedInShortestPath && styles.usedInShortestPath
                      }`}
              ...
              {edge.type === "undirected" && (
                <>
                  <path
                    ...
                    id={`${node.id}${edge.to}`}
                    onClick={() => handleEdge(edge, node)}
                    className={`${styles.undirectedEdge} ${deleteEdgeMode && styles.deleteEdgeMode
